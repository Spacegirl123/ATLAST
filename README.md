...
Applied to **5,052 asteroids** from Rubin's early datasets, AT-LAST identified **523 high-confidence rotation periods**. Among 33 objects for which earlier work reported a single preferred period, the pipeline reproduced 32 — 31 directly and one at an accepted half/double harmonic — corresponding to **97% agreement**. It also recovered previously reported ultra-fast rotators and identified additional high-confidence candidates for independent confirmation. :contentReference[oaicite:5]{index=5} :contentReference[oaicite:6]{index=6}
